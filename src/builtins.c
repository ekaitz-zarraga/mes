...
mes_builtins (struct scm *a)            /*:((internal)) */
{
  ...
  a = init_builtin (builtin_type, "hash-create-handle!", 3, &hash_create_handle_x, a);
  a = init_builtin (builtin_type, "hashq-set!", 3, &hashq_set_x, a);
  a = init_builtin (builtin_type, "hash-set!", 3, &hash_set_x, a);
  a = init_builtin (builtin_type, "hash-remove!", 2, &hash_remove_x, a);
  ...
  a = init_builtin (builtin_type, "string-copy!", -1, &string_copy_x, a);
  a = init_builtin (builtin_type, "make-string", -1, &make_string_init, a);
  /* src/struct.c */
